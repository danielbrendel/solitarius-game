...
/* Alien laser ship entity */
class CAlienLaserShip : IScriptedEntity
{
	Vector m_vecPos;
	Vector m_vecSize;
	float m_fRotation;
	...
	void CheckForEnemiesInRange()
	{
		...
		if (this.m_vecPos.Distance(pEntity.GetPosition()) <= C_ALIENLASERSHIP_REACT_RANGE) {
			this.m_bGotEnemy = true;
		}
		
		if (this.m_bGotEnemy) {
			if (this.m_fSpeed == C_ALIENLASERSHIP_DEFAULT_SPEED)
				this.m_fSpeed *= 2;
				
			this.LookAt(pEntity.GetPosition());

			if (this.m_vecPos.Distance(pEntity.GetPosition()) <= C_ALIENLASERSHIP_ATTACK_RANGE) {
