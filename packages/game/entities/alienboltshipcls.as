...
const int C_ALIENBOLTSHIP_REACT_RANGE = 500;
const int C_ALIENBOLTSHIP_ATTACK_RANGE = 350;
const float C_ALIENBOLTSHIP_DEFAULT_SPEED = 65.0;

/* Alien bolt ship entity */
class CAlienBoltShip : IScriptedEntity
{
	Vector m_vecPos;
	Vector m_vecSize;
	float m_fRotation;
	...
	void CheckForEnemiesInRange()
	{
		...
		if (this.m_vecPos.Distance(pEntity.GetPosition()) <= C_ALIENBOLTSHIP_REACT_RANGE) {
			this.m_bGotEnemy = true;
		}
		
		if (this.m_bGotEnemy) {
			if (this.m_fSpeed == C_ALIENBOLTSHIP_DEFAULT_SPEED)
				this.m_fSpeed *= 2;
				
			this.LookAt(pEntity.GetPosition());

			if (this.m_vecPos.Distance(pEntity.GetPosition()) <= C_ALIENBOLTSHIP_ATTACK_RANGE) {
	...
	CAlienBoltShip()
    {
		...
		this.m_fSpeed = C_ALIENBOLTSHIP_DEFAULT_SPEED;
		this.m_uiHealth = 90;
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_hMove = R_LoadSprite(GetPackagePath() + "gfx\\alienships\\alienlasership.png", 1, 84, 104, 1, false);
		this.m_hAttackSound = S_QuerySound(GetPackagePath() + "sound\\bolt.wav");
